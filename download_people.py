...
def read_people_df(parliament):
    df = pd.DataFrame(query_people(parliament), columns= regular_cols + death_cols)
...
def download_people(parliament):
    ...
    print("Downloading people for parliament #%d to %s..." % (parliament, filename))
    df = read_people_df(parliament)
    df.to_csv(filename, encoding='utf8')

for parliament in range(40, current_parliament + 1):
